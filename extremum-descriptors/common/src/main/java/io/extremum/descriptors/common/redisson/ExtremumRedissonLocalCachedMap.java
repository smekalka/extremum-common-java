...
public class ExtremumRedissonLocalCachedMap<K, V> extends RedissonMap<K, V> implements RLocalCachedMap<K, V> {
    ...
    public ExtremumRedissonLocalCachedMap(CommandAsyncExecutor commandExecutor, String name, LocalCachedMapOptions<K, V> options,
    ...
    public ExtremumRedissonLocalCachedMap(Codec codec, CommandAsyncExecutor connectionManager, String name, LocalCachedMapOptions<K, V> options,
                                          EvictionScheduler evictionScheduler, RedissonClient redisson, WriteBehindService writeBehindService) {
    ...
    private void init(LocalCachedMapOptions<K, V> options, RedissonClient redisson, EvictionScheduler evictionScheduler) {
        ...
        localCacheView = new FlexibleCacheView<>(options, this);
    ...
    @Override
    public RFuture<V> getAsync(Object key, long th) {
        checkKey(key);
        ...
        RFuture<V> future = super.getAsync((K) key, th);
    ...
    public RFuture<Map<K, V>> getAllAsync(Set<K> keys) {
        ...
        if (storeMode == LocalCachedMapOptions.StoreMode.LOCALCACHE) {
            ...
            if (!newKeys.isEmpty()) {
                CompletionStage<Map<K, V>> f = loadAllMapAsync(newKeys.spliterator(), false, 1)
                                                    .thenApply(r -> result);
                return new CompletableFutureWrapper<>(f);
            }
            return new CompletableFutureWrapper<>(result);
